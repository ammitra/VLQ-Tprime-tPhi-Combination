...
# )
mappable = ax.imshow(
    limits.T, cmap='viridis', norm=matplotlib.colors.LogNorm(vmin=np.nanmin(limits)+1e-1, vmax=np.nanmax(limits)), aspect='auto', origin='lower', extent=[1000,3000,25,500]
)
...
fig.savefig('plots/limits_2D_box_imshow.pdf',bbox_inches='tight')
