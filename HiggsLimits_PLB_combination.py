'''
minimal, clean plot of the combination results
'''
import os, glob
import numpy as np
import matplotlib
...
for version in ['Supplementary', 'Final', 'Preliminary']:
    ...
    ax.plot(MTs, med, color='black', linewidth=lw, linestyle='--', marker=None, label='Expected')
    ax.plot(MTs, obs, color='black', linewidth=lw, linestyle='-', marker='o', markersize=8, label='Observed')

    ax.set_ylabel(
        #r'$\sigma(pp \to T^\prime bq)$ $\mathcal{B}(T^\prime \to tH(b\overline{b}))$ [fb]',
        r'$\sigma(pp \to T^{\Huge{\prime}} bq)$ $\mathcal{B}(T^{\Huge{\prime}} \to tH(b\overline{b}))$ [fb]',
        #r'$\sigma(pp \to T^{\fontsize{30pt}{3em}\selectfont{}{\prime}} bq)$ $\mathcal{B}(T^{\fontsize{30pt}{3em}\selectfont{}{\prime}} \to tH(b\overline{b}))$ [fb]',
        fontsize=fs
    )
    ...
    # ax.plot(MTs,     B2G_22_001, color='black', linewidth=lw, linestyle='solid', label=r'$H \to b\overline{b},\,t\to bq\overline{q}^\prime$')
    # ax.plot(MTs[2:], B2G_23_009, color='black', linewidth=lw, linestyle='solid', label=r'$H \to b\overline{b},\,t\to b\ell\nu$')

    # Individual results (expected)
    # ax.plot(MTs,     B2G_22_001_exp, color='black', linewidth=lw, linestyle='dashed')
    # ax.plot(MTs[2:], B2G_23_009_exp, color='black', linewidth=lw, linestyle='dashed')


    # Plot the nominal theory xsecs for 1% and 5% width
    theory1_c = '#7a21dd' 
    theory5_c = '#e42536'
    ax.plot(x1, y1, linewidth=lw, linestyle='solid', label=r'$\Gamma/m_{T^{\prime}}=$1%', color=theory1_c)
    ax.plot(x5, y5, linewidth=lw, linestyle='solid', label=r'$\Gamma/m_{T^{\prime}}=$5%', color=theory5_c)
    ax.text(0.27, 0.95, r'Singlet VLQ benchmark', ha='center', va='top', fontsize=fs-7, transform=ax.transAxes, fontproperties='Tex Gyre Heros')
    ...
    hep.cms.label(loc=0, ax=ax, label='', rlabel='', data=True, fontsize=fs)
    ...
    plt.savefig(f"plots/HiggsLimits_combined_PLB_{version}.pdf",bbox_inches='tight')
    plt.savefig(f"plots/HiggsLimits_combined_PLB_{version}.png",bbox_inches='tight')
